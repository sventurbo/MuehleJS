...
class MuehleGame {
  ...
  getLegalActions(player = this.turn) {
    ...
    const moves = this.getLegalMoves(player);
    const actions = [];
    Object.keys(moves).forEach(from => {
      moves[from].forEach(to => actions.push({ action: 'move', player, from, to }));
    });
    return actions;
  }
  ...
  makeRandomLegalMove(player = this.turn, random = Math.random) {
    ...
    if (choice.action === 'place') {
      result = this.placePiece(player, choice.point);
    } else if (choice.action === 'move') {
      result = this.movePiece(player, choice.from, choice.to);
    } else {
      result = this.removePiece(player, choice.point);
    }
...
// Check if phase transitions to MOVING
     ...
     // The final stone of the SETTING phase may leave the opponent without a move

...
const { parseTrustProxy, getClientAddress, rateLimitKey } = require('./clientAddress');
...
class GameManager {
   ...
   * @param {string|boolean|Array<string>|Function} [options.trustProxy] - which
   *   peers may set `X-Forwarded-For`; defaults to the `TRUST_PROXY` environment
   *   variable, and to trusting nobody when that is unset.
   */
  constructor(io, options = {}) {
    this.io = io;

    // Only a configured reverse proxy may rename its clients via X-Forwarded-For.
    this.trustProxy = parseTrustProxy(
      options.trustProxy !== undefined ? options.trustProxy : process.env.TRUST_PROXY
    );
  ...
  enqueuePlayer(socket, username) {
    // 1. Rate-limit check per socket and per address block.
    // The key is folded to a /64 for native IPv6, because a client that owns a
    // whole prefix would otherwise just move to the next address (see clientAddress.js).
    const addressKey = rateLimitKey(getClientAddress(socket, this.trustProxy));
    if (this.queueLimiter.isLimited(socket.id) || this.queueIpLimiter.isLimited(addressKey)) {

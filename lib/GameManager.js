...
const { colorName } = require('../shared/muehleRules');
 ...
 * `escapeHtml()`. Escaping here as well produced literal `&amp;` and `&lt;b&gt;`
...
class GameManager {
  ...
  enqueuePlayer(socket, username) {
    ...
    // A name is required; anything longer than 12 characters is cut off, and
    // control characters are stripped before the name ever reaches a screen.
    const rawName = (typeof username === 'string') ? username.trim().substring(0, 12) : '';
    if (!rawName) return;
  ...
  _createGame(player1, player2) {
    ...
    // Each player is told the same state, only from their own side of the board.
    const initialGameState = game.getState();
    ['W', 'B'].forEach(color => {
      const you = players[color];
      const opponentColor = color === 'W' ? 'B' : 'W';
      const opponent = players[opponentColor];

      this.socketMap.set(you.socket.id, { gameId, color, username: you.username });
      you.socket.join(gameId);
      you.socket.emit('gameStart', {
        gameId,
        yourColor: color,
        yourName: you.username,
  ...
  handleTurnTimeout(gameId) {
    ...
    this.io.to(gameId).emit('turnTimeout', {
      ...
      message: `Zeit abgelaufen – für ${colorName(player)} wurde automatisch gezogen.`
    });
  ...
  /**
   * The path every board action takes: rate limit, find the session, let the
   * engine judge the action, then either tell the player why it was refused or
   * broadcast the new state to both of them.
   *
   * The engine is asked through a callback so this method never needs to know
   * which action it is running — placing, moving and capturing differ only in
   * that one line, and all three are equally untrusted until the engine says so.
   *
   * @param {Object} socket The socket that sent the action.
   * @param {Function} play Receives (game, color) and returns the engine result.
   * @returns {Object} The engine result, or a refusal of our own.
   */
  _runGameAction(socket, play) {
    ...
    const result = play(gameSession.game, playerInfo.color);
    if (!result.success) {
  ...
  /** Phase 1: the player places a stone on a free point. */
  handlePlacePiece(socket, point) {
    return this._runGameAction(socket, (game, color) => game.placePiece(color, point));
  }

  /** Phase 2 & 3: the player moves (or jumps with) one of their stones. */
  handleMovePiece(socket, from, to) {
    return this._runGameAction(socket, (game, color) => game.movePiece(color, from, to));
  }

  /** The player captures an opponent stone after closing a mill. */
  handleRemovePiece(socket, point) {
    return this._runGameAction(socket, (game, color) => game.removePiece(color, point));
  }
